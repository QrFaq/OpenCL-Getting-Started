...
int main(void)
{
    ...
    cl_context context = CHECK_clCreateContext(NULL, 1, &device_id, NULL, NULL, err_clCreateContext);

    // Create a command queue
    cl_command_queue command_queue = CHECK_clCreateCommandQueue(
        context,
        device_id,
    ...
    cl_mem a_mem_obj = CHECK_clCreateBuffer(context, CL_MEM_READ_ONLY, LIST_SIZE * sizeof(int), NULL, err_a_mem_obj);
    cl_mem b_mem_obj = CHECK_clCreateBuffer(context, CL_MEM_READ_ONLY, LIST_SIZE * sizeof(int), NULL, err_b_mem_obj);
    cl_mem c_mem_obj = CHECK_clCreateBuffer(context, CL_MEM_WRITE_ONLY, LIST_SIZE * sizeof(int), NULL, err_c_mem_obj);

    // Copy the lists A and B to their respective memory buffers
    CHECK_clEnqueueWriteBuffer(command_queue, a_mem_obj, CL_TRUE, 0, LIST_SIZE * sizeof(int), A, 0, NULL, NULL, err_clEnqueueWriteBuffer_A);
    CHECK_clEnqueueWriteBuffer(command_queue, b_mem_obj, CL_TRUE, 0, LIST_SIZE * sizeof(int), B, 0, NULL, NULL, err_clEnqueueWriteBuffer_B);

    // Create a program from the kernel source
    cl_program program = CHECK_clCreateProgramWithSource(context, 1, (const char **)&source_str, &source_size, err_clCreateProgramWithSource);
    ...
    if (ret)
    {
        ...
        size_t sizeused;
        CHECK_clGetProgramBuildInfo(program, device_id, CL_PROGRAM_BUILD_LOG, val_size - 1, //?
                                    &val, &sizeused, err_clGetProgramBuildInfo);
    ...
    // src: https://stackoverflow.com/questions/23550912/measuring-execution-time-of-opencl-kernels
    cl_event event;
    CHECK_clEnqueueNDRangeKernel(
        ...
        &local_item_size,
        0, NULL, &event,
        // checker wrapper vars
        err_clEnqueueNDRangeKernel//event
    );
